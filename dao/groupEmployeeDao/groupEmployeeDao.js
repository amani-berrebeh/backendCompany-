...
async function createGroupAndAssignEmployees(groupData, employeeIds) {
    try {
        ...
        if (Array.isArray(employeeIds) && employeeIds.length > 0) {
            await Employee.updateMany({ _id: { $in: employeeIds } }, { $set: { group: newGroup._id } });

            await groupEmployee.findByIdAndUpdate(newGroup._id, { $push: { employees: { $each: employeeIds } } });
        }

        return newGroup;
    } catch (error) {
        throw new Error('Error creating group and assigning employees: ' + error.message);
    }
}
